...
public class PlayerListener implements Listener
{
    ...
    public PlayerListener(final Werewolf plugin) {
        super();
        this.plugin = null;
    ...
    public void onPlayerJoin(final PlayerJoinEvent event) {
        ...
        this.plugin.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)this.plugin, (Runnable)new CheckTransformationTask(event.getPlayer()), 20L);
        ...
        if (this.plugin.useUpdateNotifications && (event.getPlayer().isOp() || Werewolf.getPermissionsManager().hasPermission(event.getPlayer(), "werewolf.updates"))) {
            this.plugin.getServer().getScheduler().runTaskAsynchronously((Plugin)this.plugin, (Runnable)new UpdateNotifier(this.plugin, event.getPlayer()));
        }
    }
    ...
    public void onPlayerAnimation(final PlayerAnimationEvent event) {
        ...
        if (!event.isCancelled()) {
            ...
            if (Werewolf.getWerewolfManager().hasWerewolfSkin(event.getPlayer().getName())) {
                final WerewolfSkin skin = Werewolf.getSkinManager().getSkin(event.getPlayer());
                Werewolf.getSkinManager().sendPacketsToWorld(event.getPlayer().getWorld(), new Packet[] { skin.getAnimationPacket(0) });
            }
            final Player nearestWerewolf = Werewolf.getWerewolfManager().getNearestWerewolf(event.getPlayer().getName());
            if (nearestWerewolf != null && DamageHack.canPlayerHit(event.getPlayer(), nearestWerewolf.getLocation())) {
                final int damage = DamageHack.getDamageFromItemInHand(event.getPlayer());
                nearestWerewolf.damage((double)damage, (Entity)event.getPlayer());
            }
        }
    }
    ...
    public void onHeldItemChange(final PlayerItemHeldEvent event) {
        ...
        if (Werewolf.getWerewolfManager().hasWerewolfSkin(player.getName())) {
            ...
            if (skin != null) {
                ...
                final net.minecraft.server.v1_7_R1.ItemStack craftItem = CraftItemStack.asNMSCopy(heldItem);
    ...
    public void onPlayerMove(final PlayerMoveEvent event) {
        ...
        if (this.plugin.usePounce) {
            final double diff = event.getTo().getY() - event.getFrom().getY();
            if (diff <= 0.0) {
                Werewolf.getWerewolfManager().removePouncing(player.getName());
            }
            else if (!Werewolf.getWerewolfManager().isPouncing(player.getName()) && event.getTo().getWorld().getBlockAt(event.getTo()).getType() == Material.AIR) {
        ...
        if (Math.random() < 0.05) {
            int wolfsOwned = 0;
            for (final Entity entity : player.getNearbyEntities((double)this.plugin.wolfdistance, (double)this.plugin.wolfdistance, (double)this.plugin.wolfdistance)) {
    ...
    public void onPlayerDeath(final PlayerDeathEvent event) {
        if (event.getEntity().getKiller() == null || event.getEntity().getKiller().getName() == null) {
            if (this.plugin.renameWerewolves && Werewolf.getWerewolfManager().hasWerewolfSkin(event.getEntity().getName())) {
        ...
        if (this.plugin.renameWerewolves) {
            ...
            if (Werewolf.getWerewolfManager().hasWerewolfSkin(killerName)) {
                switch (this.random.nextInt(3)) {
                    case 0: {
                        event.setDeathMessage(String.valueOf(victimName) + " was slaughtered by a Werewolf");
                        break;
                    }
                    case 1: {
                        event.setDeathMessage(String.valueOf(victimName) + " was ripped apart by a Werewolf");
                        break;
                    }
                    case 2: {
                        event.setDeathMessage(String.valueOf(victimName) + " was torn to pieces by a Werewolf");
                        break;
                    }
                }
            }
        ...
        if (this.plugin.useTrophies) {
            final Player killer = this.plugin.getServer().getPlayer(killerName);
            if (killer != null && (killer.getItemInHand().getType() == Material.DIAMOND_SWORD || killer.getItemInHand().getType() == Material.GOLD_SWORD || killer.getItemInHand().getType() == Material.IRON_SWORD || killer.getItemInHand().getType() == Material.STONE_SWORD)) {
                final ItemStack trophy = Werewolf.getTrophyManager().getTrophyFromWerewolfPlayer(killerName);
                event.getDrops().add(trophy);
            }
        }
        if (this.plugin.useClans) {
            final ClanManager.ClanType killerClan = Werewolf.getWerewolfManager().getWerewolfClan(killerName);
            final ClanManager.ClanType victimClan = Werewolf.getWerewolfManager().getWerewolfClan(victimName);
            if (killerClan == victimClan && Werewolf.getClanManager().isAlpha(killerClan, victimName)) {
    ...
    public void onPlayerPickupItem(final PlayerPickupItemEvent event) {
        ...
        if (this.plugin.keepWerewolfHandsFree && player.getItemInHand().getType() == Material.AIR) {
            event.setCancelled(true);
        }
    ...
    static /* synthetic */ int[] $SWITCH_TABLE$org$bukkit$Material() {
        final int[] $switch_TABLE$org$bukkit$Material = PlayerListener.$SWITCH_TABLE$org$bukkit$Material;
        ...
        return PlayerListener.$SWITCH_TABLE$org$bukkit$Material = $switch_TABLE$org$bukkit$Material2;
    }
