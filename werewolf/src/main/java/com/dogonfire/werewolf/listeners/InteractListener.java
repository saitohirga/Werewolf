...
public class InteractListener implements Listener
{
    ...
    public InteractListener(final Werewolf plugin) {
        super();
        this.plugin = null;
        this.plugin = plugin;
    }
    
    private boolean checkForEatingFood(final PlayerInteractEvent event) {
        final Player player = event.getPlayer();
        final Material type = player.getItemInHand().getType();
        switch ($SWITCH_TABLE$org$bukkit$Material()[type.ordinal()]) {
            case 177: {
                player.sendMessage(ChatColor.RED + Werewolf.getLanguageManager().getLanguageString(LanguageManager.LANGUAGESTRING.WerewolfTryEat));
    ...
    private boolean checkForDrinkingWerewolfCurePotion(final PlayerItemConsumeEvent event) {
        final Player player = event.getPlayer();
        ...
        if (!Werewolf.getWerewolfManager().isWerewolf(player.getName())) {
            player.sendMessage(ChatColor.LIGHT_PURPLE + Werewolf.getLanguageManager().getLanguageString(LanguageManager.LANGUAGESTRING.DrinkCureFailure));
        }
        else {
            ...
            if (Math.random() < this.plugin.cureChance) {
                ...
                player.sendMessage(ChatColor.LIGHT_PURPLE + Werewolf.getLanguageManager().getLanguageString(LanguageManager.LANGUAGESTRING.DrinkCureSuccess));
            }
            else {
                player.sendMessage(ChatColor.LIGHT_PURPLE + Werewolf.getLanguageManager().getLanguageString(LanguageManager.LANGUAGESTRING.DrinkCureFailure));
            }
    ...
    private boolean checkForDrinkingWerewolfInfectionPotion(final PlayerItemConsumeEvent event) {
        final Player player = event.getPlayer();
        final ItemStack handItem = player.getItemInHand();
        if (!handItem.hasItemMeta()) {
            return false;
        ...
        if (Werewolf.getWerewolfManager().isWerewolf(player)) {
            player.sendMessage(ChatColor.LIGHT_PURPLE + Werewolf.getLanguageManager().getLanguageString(LanguageManager.LANGUAGESTRING.DrinkInfectionFailure));
        }
        else if (this.plugin.isFullMoonInWorld(player.getWorld())) {
            player.sendMessage(ChatColor.LIGHT_PURPLE + Werewolf.getLanguageManager().getLanguageString(LanguageManager.LANGUAGESTRING.DrinkInfectionSuccess));
    ...
    public void onPlayerInteract(final PlayerInteractEvent event) {
        ...
        final Player player = event.getPlayer();
        if ((event.getAction().equals((Object)Action.RIGHT_CLICK_AIR) || event.getAction().equals((Object)Action.RIGHT_CLICK_BLOCK)) && Werewolf.getWerewolfManager().hasWerewolfSkin(player.getName())) {
            this.checkForEatingFood(event);
        }
    }
    
    @EventHandler
    public void onPlayerConsumeEvent(final PlayerItemConsumeEvent event) {
        if (!Werewolf.pluginEnabled) {
            return;
        }
        if (Werewolf.getWerewolfManager().isWerewolf(event.getPlayer())) {
            if (!this.checkForDrinkingWerewolfCurePotion(event)) {
                this.checkForDrinkingWerewolfInfectionPotion(event);
            }
        }
        else if (!this.checkForDrinkingWerewolfInfectionPotion(event)) {
            this.checkForDrinkingWerewolfCurePotion(event);
        }
    }
    
    static /* synthetic */ int[] $SWITCH_TABLE$org$bukkit$Material() {
        final int[] $switch_TABLE$org$bukkit$Material = InteractListener.$SWITCH_TABLE$org$bukkit$Material;
        ...
        return InteractListener.$SWITCH_TABLE$org$bukkit$Material = $switch_TABLE$org$bukkit$Material2;
    }
}
