...
public class InventoryListener implements Listener
{
    ...
    public InventoryListener(final Werewolf p) {
        super();
        this.plugin = null;
        this.plugin = p;
    }
    ...
    public void onPrepareItemCraft(final PrepareItemCraftEvent event) {
        if (event.getView().getType() != InventoryType.WORKBENCH) {
            this.plugin.logDebug("Not workbench. Was " + event.getView().getType().name());
            return;
        }
        this.plugin.logDebug("PrepareItemCraftEvent");
        final CraftingInventory craftingInventory = event.getInventory();
        if (!craftingInventory.contains(Material.IRON_SWORD)) {
            this.plugin.logDebug("Not sword");
            return;
        }
        if (!craftingInventory.contains(Material.QUARTZ)) {
            this.plugin.logDebug("Not quartz");
            return;
        }
        final ItemStack silverSword = new ItemStack(Material.IRON_SWORD);
        final ItemMeta itemMeta = silverSword.getItemMeta();
        final List<String> list = new ArrayList<String>();
        list.add(ChatColor.DARK_GREEN + "Good for slaying Werewolves!");
        itemMeta.setDisplayName(ChatColor.GOLD + "Werewolf Slayer");
    ...
    public void onInventoryClose(final InventoryCloseEvent event) {
        ...
        if (player.getInventory().getHelmet() != null) {
            switch ($SWITCH_TABLE$org$bukkit$Material()[player.getInventory().getHelmet().getType().ordinal()]) {
                case 215:
                case 219:
                case 223:
                case 227:
        ...
        if (player.getInventory().getBoots() != null) {
            switch ($SWITCH_TABLE$org$bukkit$Material()[player.getInventory().getBoots().getType().ordinal()]) {
                case 218:
                case 222:
                case 226:
                case 230:
    ...
    static /* synthetic */ int[] $SWITCH_TABLE$org$bukkit$Material() {
        final int[] $switch_TABLE$org$bukkit$Material = InventoryListener.$SWITCH_TABLE$org$bukkit$Material;
        ...
        return InventoryListener.$SWITCH_TABLE$org$bukkit$Material = $switch_TABLE$org$bukkit$Material2;
    }
}
