...
public class DamageHack
{
    private static /* synthetic */ int[] $SWITCH_TABLE$org$bukkit$Material;
    
    public static int getDamageFromItemInHand(final Player player) {
        int damage = 0;
        short durability = 1;
        ...
        switch ($SWITCH_TABLE$org$bukkit$Material()[mat.ordinal()]) {
            ...
            case 233:
            case 234: {
                if (Werewolf.getWerewolfManager().getNumberOfTransformations(player.getName()) < 10) {
                    damage = 8;
                    break;
                }
                damage = 1;
    ...
    public static boolean canPlayerHit(final Player player, final Location targetPoint) {
        ...
        final double dotProduct = player.getLocation().getDirection().dot(diff);
        return dotProduct > 0.95;
    }
    
    public static boolean isPlayerInCone(final Player player, final Location startpoint, final int radius, final int degrees, final int direction) {
        final List newPlayers = new ArrayList();
        final int[] startPos = { (int)startpoint.getX(), (int)startpoint.getZ() };
        final int[] endA = { (int)(radius * Math.cos(direction - degrees / 2)), (int)(radius * Math.sin(direction - degrees / 2)) };
        final Location l = player.getLocation();
        if (!isPointInCircle(startPos[0], startPos[1], radius, l.getBlockX(), l.getBlockY())) {
            return false;
        }
        final int[] playerVector = getVectorForPoints(startPos[0], startPos[1], l.getBlockX(), l.getBlockY());
        final double angle = getAngleBetweenVectors(endA, playerVector);
        return Math.toDegrees(angle) < degrees && Math.toDegrees(angle) > 0.0;
    }
    ...
    public static boolean isPointInCircle(final int cx, final int cy, final int radius, final int px, final int py) {
        final double dist = px - cx ^ 2 + (py - cy) ^ 0x2;
    ...
    static /* synthetic */ int[] $SWITCH_TABLE$org$bukkit$Material() {
        final int[] $switch_TABLE$org$bukkit$Material = DamageHack.$SWITCH_TABLE$org$bukkit$Material;
        ...
        return DamageHack.$SWITCH_TABLE$org$bukkit$Material = $switch_TABLE$org$bukkit$Material2;
    }
}
